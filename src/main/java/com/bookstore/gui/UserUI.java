...
public class UserUI implements MouseListener{
    ...
    public void orderUI()throws ClassNotFoundException, SQLException {
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            Object[] row = {user.getId(), user.getUsername(), user.getPassword(), user.getStatus(),user.getName(),user.getEmail(),user.getPhone(),user.getCreatedAt(),user.getUpdatedAt(),user.getRole()};
            data[i] = row;
        }
