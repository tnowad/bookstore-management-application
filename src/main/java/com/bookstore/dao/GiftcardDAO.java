...
public class GiftcardDAO implements DAOInterface<GiftcardModel> {
    ...
    public int insert(GiftcardModel giftcardModel) throws SQLException {
        try (
                ...
                PreparedStatement pst = conn.prepareStatement(
                        "INSERT INTO `giftcard_table` (`giftcard_id`, `email`, `value`, `redemption_history`, `expiration_date`) VALUES (?, ?, ?, ?, ?)")) {
    ...
    public int update(GiftcardModel giftcardModel) throws SQLException {
        try (
                Connection conn = DatabaseConnect.getConnection();
                PreparedStatement pst = conn.prepareStatement(
                        "UPDATE `giftcard` SET `email` = ?, `value` = ?, `redemption_history` = ?, `expiration_date` = ? WHERE `giftcard_id` = ?")) {
    ...
    public int delete(String giftcardID) throws SQLException {
        try (
                ...
                PreparedStatement pst = con.prepareStatement("DELETE FROM `giftcard` WHERE giftcard_id = ?")) {
    ...
    public List<GiftcardModel> searchByCondition(String condition) throws SQLException {
        // Build the SQL query with passed condition for searching giftcard data
        ...
        try (
            ...
            // Create an ArrayList of giftcards to hold the retrieved ones
