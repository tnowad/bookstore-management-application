...
public class PaymentDAO implements IDAO<PaymentModel> {
  ...
  private PaymentModel createPaymentModelFromResultSet(ResultSet rs)
    throws SQLException {
  ...
  public int insert(PaymentModel payment) {
    String insertSql =
      "INSERT INTO payments (order_id, user_id, amount, payment_method, payment_method_id, status) VALUES (?, ?, ?, ?, ?, ?)";
    Object[] args = {
      payment.getOrderId(),
      payment.getUserId(),
      payment.getAmount(),
  ...
  public int update(PaymentModel payment) {
    String updateSql =
      "UPDATE payments SET order_id = ?, user_id = ?, amount = ?, payment_method = ?, payment_method_id = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
    Object[] args = {
      payment.getOrderId(),
      payment.getUserId(),
      payment.getAmount(),
  ...
  public List<PaymentModel> search(String condition, String[] columnNames) {
    ...
    if (columnNames == null || columnNames.length == 0) {
      // Search all columns
      query =
        "SELECT * FROM payments WHERE CONCAT(id, order_id, user_id, amount, payment_method, payment_method_id, status, created_at, updated_at) LIKE ?";
      ...
      query =
        "SELECT id, order_id, user_id, amount, payment_method, payment_method_id, status, created_at, updated_at FROM payments WHERE CONCAT(" +
