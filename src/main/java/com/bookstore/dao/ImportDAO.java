...
public class ImportDAO implements IDAO<ImportModel> {
  ...
  private ImportModel createImportModelFromResultSet(ResultSet rs)
    throws SQLException {
    int id = rs.getInt("id");
  ...
  public int insert(ImportModel imports) {
    String insertSql =
      "INSERT INTO imports (provider_id, employee_id, total_price) VALUES (?, ?, ?)";
    Object[] args = {
      imports.getProviderId(),
  ...
  public int update(ImportModel imports) {
    String updateSql =
      "UPDATE imports SET provider_id = ?, employee_id = ?, total_price = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
    Object[] args = {
      imports.getProviderId(),
      imports.getEmployeeId(),
      imports.getTotalPrice(),
  ...
  public List<ImportModel> search(String condition, String[] columnNames) {
    ...
    if (columnNames == null || columnNames.length == 0) {
      // Search all columns
      query =
        "SELECT * FROM imports WHERE CONCAT(id, provider_id, employee_id, total_price, created_at, updated_at) LIKE ?";
      ...
      query =
        "SELECT id, provider_id, employee_id, total_price, created_at, updated_at FROM imports WHERE CONCAT(" +
